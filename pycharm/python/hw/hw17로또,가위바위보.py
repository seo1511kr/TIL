...
#가위바위보에서 인덱스 차이를 활용해서 규칙을 찾아서 해결하는 방법도 있음
...
#     for i in num_li:
#         count=0
#         for j in lotto_num: # set으로 교집합 활용하면 시간복잡도 좋아짐;
#             if j in i:count+=1
#         if count == 6:
...
#  함수 기능을 분리하여 각각 입력 처리 출력을 분리해주는 것도 좋음

# if문에서
# if 조건:출력
# else:출력
# 의 방식에서 if에 들어가는 조건이 더 포괄적일 수록 효율적임
